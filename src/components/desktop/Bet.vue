  ...
  <div class="px-[10px] py-[5px] bg-lighter-warning dark:bg-light-dark rounded-[5px]">
    <div class="flex justify-between items-center">
      <div class="flex items-center">
        ...
        <span class="font-semibold text-sm text-header-bg dark:text-white">1 Click</span>
      </div>
      <div class="flex h-[22px] items-center font-semibold text-xs text-header-bg dark:text-white">
        ...
        <div :class="{ 'hidden': !flag, 'flex': flag }">
          <button
            class="h-[22px] px-[10px] bg-warning rounded-[5px] dark:text-header-bg"
            :class="{
              'hidden': editFlag,
              'block': !editFlag
            }"
          ...
          <div :class="{ 'hidden': !editFlag, 'flex': editFlag }">
            <button
              class="h-[22px] px-[10px] bg-warning rounded-[5px] mr-[5px] dark:text-header-bg"
              @click="save()"
            >Save</button>
            <button
              class="h-[22px] px-[10px] bg-danger rounded-[5px] text-white"
              @click="cancel()"
            >Cancel</button>
          </div>
        </div>
